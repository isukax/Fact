...
void Application::ProcessNode(aiNode* node, const aiScene* scene)
{
#define OPTIMIZE_MESH_RUNTIME 1
	for (uint32_t i = 0; i < node->mNumMeshes; i++)
	{
		if (i > 0) return;
		aiMesh* mesh = scene->mMeshes[node->mMeshes[i]];
		ProcessMesh(mesh, scene, vertices, indices, OPTIMIZE_MESH_RUNTIME);
...
void Application::ProcessMesh(aiMesh* mesh, const aiScene* scene, std::vector<Vertex>& outVertices, std::vector<uint32_t>& outIndices, bool execOptimize)
{
	// Walk through each of the mesh's vertices

	// TODO Optmize mesh option / meshoptimizer
	...
	for (uint32_t i = 0; i < mesh->mNumVertices; i++)
	{
		...
		if (mesh->mTextureCoords[0])
		{
			...
			// fit for .obj
			vertex.texCoord.y = 1.0f - (float)mesh->mTextureCoords[0][i].y;
		}
...
// TODO Platform / Intermediate Commands
void Application::CreateInstance()
{
	...
	// global extensions
	const std::vector<const char*> extensions = std::move(GetRequiredExtensions());
...
{
	const SwapChainSupportDetails swapChainSupport = std::move(QuerySwapChainSupport(physicalDevice));
	...
	const uint32_t queueFamilyIndices[] = { QFIndices.graphicsFamily.value(), QFIndices.presentFamily.value() };
...
{
	const QueueFamilyIndices queueFamilyIndices = std::move(FindQueueFamilies(physicalDevice));
...
void Application::RecordCommandBuffer(VkCommandBuffer InCommandBuffer, uint32_t imageIndex)
{
...
void Application::CreateTextureImage(const std::string& fileName)
{
	int texWidth, texHeight, texChannels;
	stbi_uc* pixels = stbi_load(fileName.c_str(), &texWidth, &texHeight, &texChannels, STBI_rgb_alpha);
	VkDeviceSize imageSize = texWidth * texHeight * 4;	// 4 fixed channels
	...
	CreateImage(texWidth, texHeight, texMipLevels, VK_SAMPLE_COUNT_1_BIT, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_TRANSFER_SRC_BIT | VK_IMAGE_USAGE_SAMPLED_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, textureImage, textureImageMemory);
	...
	//TransitionImageLayout(textureImage, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, texMipLevels);
...
void Application::CreateTextureSampler()
{
	...
	//samplerInfo.minLod = static_cast<float>(texMipLevels / 2);
...
void Application::TransitionImageLayout(VkImage image, VkFormat, VkImageLayout oldLayout, VkImageLayout newLayout, uint32_t mipLevels)
{
	VkCommandBuffer commandBuffer = BeginSingleTimeCommands();
...
{
	const std::string MeshPath = "../asset/meshes/viking_room.obj";
	const std::string TexturePath = "../asset/textures/viking_room.png";
